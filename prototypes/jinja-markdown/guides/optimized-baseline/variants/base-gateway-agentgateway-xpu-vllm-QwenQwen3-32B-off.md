<!-- GENERATED FILE — DO NOT EDIT. Variant: infra_provider=base router_mode=gateway gateway_provider=agentgateway accelerator=xpu model_server=vllm model=Qwen/Qwen3-32B monitoring=off. Regenerate with: guidegen.py render-md <guide dir> -->
> **Configuration:** infra_provider=base router_mode=gateway gateway_provider=agentgateway accelerator=xpu model_server=vllm model=Qwen/Qwen3-32B monitoring=off — [all configurations](../readonly-guide.md)
...
kubectl rollout status deployment -l app.kubernetes.io/instance=${GUIDE_NAME} \
  -n ${NAMESPACE} --timeout=300s
```
### 2. Deploy the Model Server (xpu / vllm)
